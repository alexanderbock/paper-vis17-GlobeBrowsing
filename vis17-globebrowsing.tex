...
The mapping of globes, that is, planets, moons, and other nearly elliptical celestial objects within our solar system is carried out by scientists of space organizations. The gathered image data are used in the planning of future space missions but seldom makes its way to the general public even though NASA provides a most of it openly.
Much work has been done in the field of terrain and globe rendering and current technologies rely on techniques such as out-of-core, dynamic level-of-detail rendering, multi-threaded data management, and considerations in precision limitations.
...
We present an application for visualizing the same data that space scientists analyze in the context of a virtual environments representing the space that we are exploring. Given the contextualization that the real space provides, we can model the solar system using positional data for modeling the planets' orbits together with the space probes that explores them.

The main focus of this paper is to describe the combination of different datasets gathered for mapping out planet surface features, particularly for Mars thanks to the extensive amount of map data gathered by the various missions for this planet.
Our rendering system combines various resolutions of local and global map datasets and presents them together in their real context. Using the Geospatial Data Abstraction Library (GDAL), we can preprocess images to match the most common cylindrical projection formats used for globe rendering. A chunked level-of-detail approach is used for rendering tiles which are height mapped dynamically. This allows for a versatile globe rendering that easily can load new datasets without requiring preprocessing by the rendering software.
The same data that scientists use in their research can be used for public dissemination and present it contextualized using immersive rendering systems. Our software, OpenSpace, is an open source software with the goal of bringing space science to the general public.

\kallecomment{Please bash my text and make it better :).
Are we focusing on the right things for the abstract?}
\textcolor{red}{What are the contributions of this paper? What will the reader learn about this application?}
%
} % end of abstract
