...
Using the inverse projection $P^{-1}$, vertices of a given chunk are unprojected from the georeferenced projection to the model space of the globe.
...
Rendering time varying layers works less like a streaming process and more like a time interval process.
A specific, ``temporal'' tile provider has the ability to create its own tile providers, where each one corresponds to its own time interval.
When the global time of the software gets to a different time interval, the temporal tile provider will start instantiating tile reading calls to a different tile provider corresponding to that interval.
This can undoubtably lead to a lot of reading within a short amount of time so memory buffering becomes important for temporal datasets.
