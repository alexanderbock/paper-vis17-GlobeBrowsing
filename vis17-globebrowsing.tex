...
%% Abstract section.
\abstract{%
We present an application that accurately contextualizes geospatial data of celestial bodies in order to enrich the experience in public dissemination, as well as support communication between domain scientists.
Mapping of the planets and moons within our solar system is carried out by  the space organizations who often share this map data openly for usage in scientific research and mission planning.
However, this data is of less relevance to non-experts as it is difficult to comprehend the context of the data and understand the acquisition process from satellites and spacecraft.
Furthermore, traditional dissemination pathways have a long leadtime and thus increase the time between discovery and dissemination.
In our interactive application, multiple types of datasets and visualization techniques are tailored and combined in order to make this data available to the general public.
We describe the image acquisition, required preprocessing steps to derive a 2.5D terrain model, and a chunked level-of-detail rendering approach to enable interactive exploration of global maps as well as local high-resolution digital terrain models with textures and height information.
...
A second case are dynamical processes, such as weather conditions on Earth, that require the support of temporal datasets.
In addition to a visualization of the finished results, we also provide the ability to show the ongoing acquisition process, exemplified in the Pluto flyby of New Horizons.
