...
%% Abstract section.
\abstract{We present work on tailoring and integration of multiple data handling and visualization methods to accurately and interactively contextualize geospatial data of celestial bodies.
The primary target use is in science communication, but also communication between domain scientists is one of the use scenarios.
Mapping of the planets and moons within our solar system is carried out by the space organizations who often share data openly for use in scientific research and mission planning.
However, this data is, in its raw format, not accessible to non-experts, as it is difficult to grasp the context of the data and the intricate acquisition process from satellites and spacecraft. As our approach is full interactive and handles dynamic data sources, we are significantly shortening the time between discovery and dissemination of data and results.
We describe the image acquisition, required pre-processing steps to derive a 2.5D terrain model, and a chunked level-of-detail rendering approach to enable interactive exploration of global maps as well as local high-resolution digital terrain models with textures and height information.
One particularly interesting case is Mars as extensive amount of map data is available, gathered by the various missions of the last 40 years.
A second case is dynamical processes, such as weather conditions on Earth, that require the support of spatio-temporal datasets.
In addition to a visualization of the scientific results, we also provide the ability to show the ongoing data acquisition process, exemplified in the Pluto flyby by New Horizons spacecraft.
Our work has been implemented in the open source software OpenSpace that enables interactive presentations in a range of environment such as, immersive dome theaters and virtual reality headsets.
